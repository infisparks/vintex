...
export default function StockLedgerPage() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const [tList, iList, vList] = await Promise.all([
        getInventoryTransactions(),
        getInventoryItems(),
        getVendors(),
      ]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <div className="flex items-center space-x-2 text-xs font-bold text-slate-400 mb-1">
            ...
            <span>/</span>
            <span className="text-slate-700">Stock Movement Ledger</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight flex items-center space-x-2.5">
            ...
            <span>Stock Movement & Inward/Outward Audit Ledger</span>
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Real-time audit log of stock movements, inward supplier batches, and customer sale deductions.
          </p>
        </div>
