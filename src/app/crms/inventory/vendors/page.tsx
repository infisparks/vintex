...
import { Vendor, getVendors } from "@/lib/firebase";
...
export default function VendorsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F6F8] text-slate-900 font-sans p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <div className="flex items-center space-x-2 text-xs font-bold text-slate-400 mb-1">
            ...
            <span>/</span>
            <span className="text-slate-700">Vendor Registration</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight flex items-center space-x-2.5">
            ...
            <span>Vendor Registration & Supplier Directory</span>
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Manage procurement vendor accounts, contact persons, GSTIN, PAN numbers and banking information.
          </p>
        </div>
      ...
      {/* Vendors Registry View Component */}
      <VendorsRegistryView
        vendors={vendors}
        isLoading={isLoading}
        onRefresh={loadData}
